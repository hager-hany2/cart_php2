...
function getspecific_user($Email,$name,$password){
    $con =contactToDB();
    
    $data= $con->query('SELECT * FROM user  WHERE email="'.$Email.'" and  name="'.$name.'"and password="'.$password.'"');
    return $data ->fetch();
}
function getspecific_user2($Email,$name,$password){
    $con =contactToDB();
    
    $data= $con->query('SELECT * FROM admin  WHERE email="'.$Email.'" and  name="'.$name.'"and password="'.$password.'"');
    return $data ->fetch();
}
function getspecific_user3($Email,$name,$password){
    $con =contactToDB();
